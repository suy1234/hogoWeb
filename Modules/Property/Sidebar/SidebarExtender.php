...
namespace Modules\Property\Sidebar;
...
class SidebarExtender extends BaseSidebarExtender
{
    ...
    {
        $menu->group(trans('sidebar.property'), function (Group $group) {
            $group->weight(1);
            $group->authorize(
            ...
            $group->item(trans('product::sidebar.product'), function (Item $item) {
                ...
                $item->item(trans('product::sidebar.category'), function (Item $item) {
                    $item->weight(2);
                    $item->route('admin.categorys.index', ['code' => 'attribute']);
                    $item->authorize(
                ...
                $item->item(trans('product::sidebar.group'), function (Item $item) {
                    $item->weight(3);
                    $item->route('admin.groups.index', ['code' => 'attribute']);
                    $item->authorize(
                ...
                $item->item(trans('product::sidebar.attribute'), function (Item $item) {
                    $item->weight(4);
                    $item->route('admin.attributes.index', ['code' => 'attribute']);
                    $item->authorize(
